...
bool Person::draw(std::vector<std::string> &myScreenVector){			//pure virtual, abstract base class, MUST BE DEFINED BY DERIVED CLASSES	
	...
	case LEFT:
		...
		myScreenVector[myScreenVector.size()-10].replace(getX(), 70, "   \\\\\\\\|//// ");
		myScreenVector[myScreenVector.size()-9].replace(getX(), 70,  "    |~ ////  ");
		myScreenVector[myScreenVector.size()-8].replace(getX(), 70,  "    |O  //   ");
		myScreenVector[myScreenVector.size()-7].replace(getX(), 70,  "   <    |    ");
		myScreenVector[myScreenVector.size()-6].replace(getX(), 70,  "    |_/ |    ");
		myScreenVector[myScreenVector.size()-5].replace(getX(), 70,  "--o |__/     ");
		myScreenVector[myScreenVector.size()-4].replace(getX(), 70, "   \\__|      ");
		myScreenVector[myScreenVector.size()-3].replace(getX(), 70,  "      |      ");
		if (bLegChangePosition)
		{
			myScreenVector[myScreenVector.size() - 2].replace(getX(), 70, "     /|      ");
			myScreenVector[myScreenVector.size() - 1].replace(getX(), 70, "   \\/_|      ");
		}
		else
		{
			myScreenVector[myScreenVector.size() - 2].replace(getX(), 70, "      |      ");
	...
	case RIGHT:
		...
		myScreenVector[myScreenVector.size()-8].replace(getX(), 100,    "   \\\\  O|    ");
		myScreenVector[myScreenVector.size()-7].replace(getX(), 100,      "    |    >   ");
		myScreenVector[myScreenVector.size()-6].replace(getX(), 100,     "    | \\_|    ");
		myScreenVector[myScreenVector.size()-5].replace(getX(), 100,     "     \\__| o--");
		myScreenVector[myScreenVector.size()-4].replace(getX(), 100,      "      |__/   ");
		myScreenVector[myScreenVector.size()-3].replace(getX(), 100,      "      |      ");
		if (bLegChangePosition)
		{
			myScreenVector[myScreenVector.size() - 2].replace(getX(), 100, "      |\\     ");
			myScreenVector[myScreenVector.size() - 1].replace(getX(), 100, "      |_\\/    ");
		}
		else
		{
			myScreenVector[myScreenVector.size() - 2].replace(getX(), 100, "      |     ");
